...
What this gives you:
- Loud, step-by-step logs for each phase (Underlying -> Definitions -> Planning -> Timeseries Pulls -> Compute -> Insert)
...
NO STRUCTURE CHANGE: same phases + loops.
CHANGE: PHASE2 batch downloads replaced by per-window timeseries.get_range pulls.
"""
...
# NOTE: you said you want manual symbols now; this import can be removed if unused
# from execution_functions import get_all_symbolsTester

client = db.Historical(DATABENTO_API_KEY)

DB_PATH = "options_data.db"
BATCH_DIR = pathlib.Path("batch_downloads")  # kept (no structure change), but not used now
DEBUG_DIR = pathlib.Path("debug_dumps")

# Databento batch symbol cap (hard limit) - kept (no structure change), not used now
...
# Optional: cap raw symbols per window pull (set None for no cap)
RAW_SYMBOLS_PER_WINDOW_CAP = None  # e.g. 800 for testing
...
def get_contract_data_from_dfs_fast(
    ...
    """
    Same output as before, but uses def_map lookup and expects dfs that already cover the needed time range.
    """
    symbols = {}
    for strike, side in strike_sides:
...
def iter_time_chunks(start, end, step: timedelta):
    cur = start
    while cur < end:
...
def iter_symbol_chunks(symbols: list[str], n: int):
    for i in range(0, len(symbols), n):
        yield symbols[i : i + n]


# ---------- TIMESERIES HELPER (NEW) ----------
def ts_get_df(dataset: str, schema: str, stype_in: str, symbols: list[str], start, end) -> pd.DataFrame:
    if not symbols:
        return pd.DataFrame()
    try:
        ...
        log(f"❌ get_range failed schema={schema} syms={len(symbols)} start={start} end={end} err={e}")
        log(traceback.format_exc())
        return pd.DataFrame()


# ---------- ✅ SHARD TWO-PHASE (defs batched; data is timeseries per window) ----------
def run_shard_two_phase(
    ...
    *,
    ...
    log(f"[PARAM] DEF_CHUNK_SIZE={DEF_CHUNK_SIZE} RAW_SYMBOLS_PER_WINDOW_CAP={RAW_SYMBOLS_PER_WINDOW_CAP}")
    ...
    # ---------- PHASE 1A: underlying per symbol ----------
    ...
    log(f"[INFO] shard union raw_symbols={len(union_raw_list):,} (NOT pre-downloaded now; pulled per-window)")
    log(f"[INFO] union_raw sample={union_raw_list[:10]}")

    # ---------- PHASE 2: (kept) now basically "timeseries mode" ----------
    with Timer("PHASE2 timeseries mode (no global downloads)"):
        log("[INFO] Skipping batch/global downloads. Will pull per-symbol per-window via timeseries.get_range.")

    # ---------- PHASE 3: per symbol compute + insert; pulls per-window ----------
    with Timer("PHASE3 compute + insert"):
        for si, (symbol, plan) in enumerate(plans.items(), start=1):
            ...
            for window_idx, (window_start, window_df) in enumerate(data_10m.groupby(pd.Grouper(freq="15min")), start=1):
                if window_df.empty:
                    continue
                ...
                # ✅ NO STRUCTURE CHANGE: this replaces the old "filter GLOBAL dfs" block
                with Timer(f"{symbol} window {window_idx} timeseries pulls"):
                    raw_list = sorted(raw_needed)
                    if RAW_SYMBOLS_PER_WINDOW_CAP is not None:
                        raw_list = raw_list[: int(RAW_SYMBOLS_PER_WINDOW_CAP)]
                        log(f"[CAP] {symbol} window {window_idx}: raw_list capped to {len(raw_list)}")

                    mkt_df_win = ts_get_df(
                        dataset="OPRA.PILLAR",
                        schema="cbbo-1m",
                        stype_in="raw_symbol",
                        symbols=raw_list,
                    ...
                    mkt_tcol = "ts_event" if "ts_event" in mkt_df_win.columns else ("timestamp" if "timestamp" in mkt_df_win.columns else None)
                    trd_tcol = "ts_event" if "ts_event" in trd_df_win.columns else ("timestamp" if "timestamp" in trd_df_win.columns else None)
                    oi_tcol = "ts_event" if "ts_event" in oi_df_win.columns else ("timestamp" if "timestamp" in oi_df_win.columns else None)

                    if mkt_tcol:
                        _ensure_utc_col(mkt_df_win, mkt_tcol)
                    if trd_tcol:
                        _ensure_utc_col(trd_df_win, trd_tcol)
                    if oi_tcol:
                        _ensure_utc_col(oi_df_win, oi_tcol)
                    ...
                    if debug_dump:
                        # keep dumps small
                        (mkt_df_win.head(200000)).to_csv(DEBUG_DIR / f"{symbol}_mkt_win_{window_idx}_head.csv", index=False)
                        (trd_df_win.head(200000)).to_csv(DEBUG_DIR / f"{symbol}_trd_win_{window_idx}_head.csv", index=False)
                        (oi_df_win.head(200000)).to_csv(DEBUG_DIR / f"{symbol}_oi_win_{window_idx}_head.csv", index=False)
            ...
            df["timestamp"] = (
                pd.to_datetime(df["timestamp"], utc=True, errors="coerce").dt.tz_convert("UTC").dt.tz_localize(None)
            )
